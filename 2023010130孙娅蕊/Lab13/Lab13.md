...
| 测试 | 预期 | 你的结果 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |返回 HTTP 目录页面 |
| `siteA -> siteB:2222` | 成功 |返回 HTTP 目录页面 |
| `siteA -> siteB ping` | 成功 |3 包收发，0% 丢包 |
...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` | 仅允许 10.1.0.0/24 访问 10.2.0.2 TCP 8080 新建连接|A-TO-B-DENY | FORWARD DROP|
| `gwB` | 仅允许 10.1.0.0/24 访问 10.2.0.2 TCP 8080 新建连接|VPN-S2S-DENY |FORWARD DROP |
...
| 测试 | 预期结果 | 实际结果 | 日志 |
| :--- | :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |正常返回网页 | 无拒绝日志|
| `siteA -> siteB:2222` | 失败 |curl 连接超时拒绝 |产生 A-TO-B-DENY 日志 |
| `siteA -> siteB ping` | 失败 |目的端口不可达 |产生 A-TO-B-DENY 日志 |
...
| underlay 是否连通 |连通，gwA/gwB 可互相 ping 通 |
...
| `latest handshake` |存在握手时间戳 | 存在握手时间戳|
| `transfer` 计数 | 收发数据包非 0| 收发数据包非 0|
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功| ESTABLISHED 放行 + 8080 新建连接 ACCEPT|
| `siteA -> siteB:2222` | 失败|LOG 记录 + REJECT 拒绝 |
| `siteA -> siteB ping` | 失败| LOG 记录 + REJECT 拒绝|

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay |192.0.2.1/192.0.2.2 | UDP 51820|加密外层隧道报文 |
| `wg0` |10.1.0.2/10.2.0.2 |TCP | 解密后的原始内网业务流量|
| B 站点 LAN |10.1.0.2/10.2.0.2 |TCP 8080 |放行后转发至业务服务器 |
...
答：远程接入：单人外网接入企业内网，客户端独立；站点到站点：两个完整内网网关互联，内网主机无感互通。AllowedIPs 配置、使用场景完全不同。

2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
答：AllowedIPs 两个作用：①匹配目标网段，自动下发路由，流量转发至隧道；②校验入站数据包源 IP 合法性，防止隧道侧流量伪造。填写对端内网才能双向互通。

3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
答：网关需要跨接口转发流量（内网↔wg0 隧道），关闭转发则跨网段数据包直接丢弃，无法完成隧道转发。

4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
答：内网主机访问异地网段无直连路由，所有跨站点流量交给本地网关，由网关通过 WireGuard 隧道转发。不配置则无法找到下一跳。

5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
答：①检查网关 IP 转发是否开启；②核对两端 AllowedIPs 网段是否正确；③查看两端路由是否指向 wg0；④检查 iptables FORWARD 防火墙是否拦截；⑤内网主机默认网关是否正确；⑥抓包验证隧道封装 / 解封装。

6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
答：好处：纵深防御，一端配置错误时另一端仍可防护内网；双向日志审计，便于定位拦截位置。仅单侧配置可行，但存在安全单点故障，不符合安全规范。

7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？
答：隐藏：内网私有 IP、业务端口、TCP 应用载荷；暴露：两端公网 /underlay IP、WireGuard 固定 UDP 51820 端口、数据包长度。

8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？
答：①gwA peer AllowedIPs 保留 10.2.0.0/24；gwB peer AllowedIPs 保留 10.1.0.0/24；
②gwB 增加 FORWARD 放行规则：wg0 出向访问 siteA 对应端口；
③gwA 增加 FORWARD 放行规则：wg0 入向流量访问 siteA 目标端口；
④两端均保留 ESTABLISHEDRELATED 回程放行规则。

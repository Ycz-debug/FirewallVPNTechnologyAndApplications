...
| :--- | :--- |
| ping 是否有回应 |无回应 |
| `wan` 收到的请求源地址 |192.0.1.1 |
| `wan` 无法回复的原因 | ① 目标主机 192.0.2.2 未配置到 lan 网段的回程路由；② 中间防火墙（fw）未开启 FORWARD 转发或配置了拒绝规则；③ NAT 或地址转换规则未正确配置，导致 wan 侧无法识别并响应 lan 侧的 ICMP 请求。|
...
答：额外记录了NAT 地址映射关系（内网真实地址、转换后的外网地址 / 端口 / 会话标识），用于回包时还原地址。说明的问题：conntrack 是 Linux 连接跟踪的核心，是状态防火墙和 NAT 功能的共同底层支撑。


2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
答：NAT 通过为两条连接分配不同的临时端口 / 会话标识，用「外网 IP + 临时端口 / 会话标识」作为区分键，匹配回包并还原给对应内网主机。
...
| TCP 38070 访问是否成功 |成功 |
| ping 是否成功 | 失败|
| ping 失败的原因 |防火墙 FORWARD 链仅允许 TCP 38070 的新连接，未放行 ICMP 协议的流量，因此 ping 包被 DROP 丢弃。 |
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：私有地址属于保留网段，不会在公网路由中被转发，公网路由器无法识别和路由这些地址，因此无法直接访问内网主机。
NAT（网络地址转换）通过将内网私有地址映射为防火墙 / 路由器的公网地址，让内网主机以公网地址身份访问互联网；同时维护地址映射表，将外网回包转发给对应的内网主机，实现内网主机的间接访问。

2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：SNAT --to-source：适用于静态公网 IP场景，公网 IP 固定不变，配置简单高效，适合固定 IP 的服务器 / 专线环境。
MASQUERADE：适用于动态公网 IP场景（如家用 PPPoE 拨号），会自动适配变化的公网 IP，无需手动修改配置，适合 IP 不固定的环境。

3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：对需要在载荷中携带 IP 地址 / 端口信息的协议（如 FTP、SIP、H.323、IPsec）造成麻烦。
原因：NAT 只修改 IP 头 / 端口，不会修改应用层载荷里的地址信息，导致外网回包无法根据载荷中的内网地址正确路由，需要应用层网关（ALG）辅助处理才能正常工作。

4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：conntrack 是连接跟踪的核心，同时为状态检测和 NAT 提供基础：
状态检测依赖 conntrack 判断连接状态，放行合法回包；
NAT 依赖 conntrack 维护地址 / 端口映射，实现回包地址还原。


集成在一起可以共享 conntrack 状态表，减少重复处理，提升性能，同时简化配置，是家用路由器 “状态防火墙 + NAT” 一体化设计的基础。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：在 FORWARD 链中添加允许 ICMP 流量的规则，示例命令：sudo ip netns exec fw iptables -A FORWARD -p icmp -j ACCEPT
